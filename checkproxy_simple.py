...
async def test_proxy(client: httpx.AsyncClient, proxy: Proxy) -> Optional[ProxyResult]:
    """Test a single proxy"""
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            try:
                ...
                result.error = f"Speed test error: {str(e)}"
        else:
...
async def run_simple_checker(workers: int = 200):
    """Run the simple proxy checker"""
    console = Console()
        ...
        with Progress(
            ...
            async def bound_check(proxy: Proxy):
                async with semaphore:
                    result = await test_proxy(client, proxy)
                    if result:
                        # Update database
    ...
    # Display results
    display_simple_results(results, sorted_proxies, console, start_time)
    
    # Save results
    save_simple_results(results, sorted_proxies)

def display_simple_results(results: List[ProxyResult], sorted_proxies: List[ProxyHistory], console: Console, start_time: float):
...
def save_simple_results(results: List[ProxyResult], sorted_proxies: List[ProxyHistory]):
    """Save results to CSV and text files"""
    if not sorted_proxies:
        return
    
    # Save comprehensive CSV with all proxy data
    csv_filename = "simple_checked_proxies.csv"
    ...
    for protocol in ['http', 'socks4', 'socks5']:
        ...
        if protocol_proxies:
            filename = f"{protocol}_simple_checked.txt"
    ...
    print(f"  • http_simple_checked.txt (top 200 HTTP proxies)")
    print(f"  • socks4_simple_checked.txt (top 200 SOCKS4 proxies)")
    print(f"  • socks5_simple_checked.txt (top 200 SOCKS5 proxies)")
...
if __name__ == "__main__":
    ...
    print(f"Simple Proxy Checker for Megabasterd")
    ...
    asyncio.run(run_simple_checker(workers))
