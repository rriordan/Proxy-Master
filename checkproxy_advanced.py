...
QUICK_TIMEOUT = 2  # Very fast timeout for initial check
SPEED_TIMEOUT = 10  # Moderate timeout for speed test
MAX_WORKERS = 500  # Higher concurrency for faster processing

# Larger speed test for better sustained speed measurement
SPEED_TEST_URL = "https://httpbin.org/bytes/51200"  # 50KB test (10x larger)

# Performance scoring weights
SPEED_WEIGHT = 0.7  # Speed is more important for file downloads
LATENCY_WEIGHT = 0.3  # Latency is less critical for sustained downloads
    ...
    def update_proxy(self, result: ProxyResult):
        ...
        if result.address not in self.history:
            ...
            if result.is_working:
                ...
                # Update average speed (weighted by number of tests)
        ...
        if result.address in self.history:
            ...
            if proxy.successful_tests > 0:
                # Normalize speed (0-100 scale, assuming 100 Mbps is excellent)
                speed_score = min(100, (proxy.avg_speed_mbps / 100) * 100)
                # Normalize latency (0-100 scale, lower is better, assuming 1s is poor)
                latency_score = max(0, 100 - (proxy.avg_response_time * 100))
...
async def speed_test(client: httpx.AsyncClient, proxy: Proxy) -> Tuple[float, float]:
    """Speed test with larger file for better sustained speed measurement"""
    try:
...
async def check_proxy_advanced(client: httpx.AsyncClient, proxy: Proxy) -> Optional[ProxyResult]:
    """Advanced proxy checking with performance scoring"""
    ...
    # Speed test for working proxies
    speed_mbps, response_time = await speed_test(client, proxy)
    ...
    if speed_mbps > 0:
        ...
        speed_score = min(100, (speed_mbps / 100) * 100)  # Normalize to 0-100
        latency_score = max(0, 100 - (response_time * 100))  # Lower latency = higher score
...
async def run_advanced_checker(workers: int = 200):
    """Run the advanced proxy checker"""
    console = Console()
    ...
    console.print(f"[green]Speed test: {SPEED_TEST_URL}[/green]")
        ...
        with Progress(
            ...
            async def bound_check(proxy: Proxy):
                async with semaphore:
                    result = await check_proxy_advanced(client, proxy)
                    if result:
                        # Update database
    ...
    # Display results
    display_advanced_results(results, sorted_proxies, console, start_time)
    
    # Save results
    save_advanced_results(results, sorted_proxies)

def display_advanced_results(results: List[ProxyResult], sorted_proxies: List[ProxyHistory], console: Console, start_time: float):
...
def save_advanced_results(results: List[ProxyResult], sorted_proxies: List[ProxyHistory]):
    """Save results to CSV and text files"""
    if not sorted_proxies:
        return
    
    # Save comprehensive CSV with all proxy data
    csv_filename = "advanced_checked_proxies.csv"
    ...
    for protocol in ['http', 'socks4', 'socks5']:
        ...
        if protocol_proxies:
            filename = f"{protocol}_advanced_checked.txt"
    ...
    print(f"  • http_advanced_checked.txt (top 200 HTTP proxies)")
    print(f"  • socks4_advanced_checked.txt (top 200 SOCKS4 proxies)")
    print(f"  • socks5_advanced_checked.txt (top 200 SOCKS5 proxies)")
...
if __name__ == "__main__":
    ...
    print(f"Advanced Proxy Checker for Megabasterd")
